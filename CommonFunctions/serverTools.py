...
class ServerTools:
    ...
    async def RestartServer(self, seconds = 120, reason=""):
        ...
        try:
            ...
            self.rcon.command(f"say Server restarting in {seconds/60} minutes due to {reason}")
            q.sendLogMessage(f"Server restarting in {seconds/60} minutes. ({seconds} seconds) (Reason: {reason})", type="Info")
